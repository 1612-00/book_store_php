<?php 
    ...
    $nameErr = $addressErr =  $telErr = $emailErr = $typeErr = "";
    $name = $address = $tel = $email = $type = $desc = "";
    ...
    // ----------------------------------------------CHECK DATA POSTED-----------------------------------------
        ...
        if(empty($_POST["address"])) {
            $addressErr = "Thiếu địa chỉ.";
        }   else {
            $address = test_input($_POST["address"]);
        }

        if(empty($_POST["tel"])) {
            $telErr = "Thiếu số điện thoại.";
        }   else {
        ...
        if($name !== "" && $address !== "" && $tel !== "" && $email !== "" && $type !== "") {

            $sql = "INSERT INTO tickets (name, address, email, tel, type, note, isChecked) VALUES ('$name', '$address', '$email', '$tel', '$type', '$note', 0)";

            if(mysqli_query($db, $sql)) {
            echo '<script type="text/javascript">alert("Gửi yêu cầu thành công, chúng tôi sẽ liên lạc với bạn sớm nhất có thể!")</script>';
            } else {
            echo '<script type="text/javascript">alert("Yêu cầu thất bại, xin hãy liên hệ với chúng tôi nếu bạn vẫn gặp tình trạng này!")</script>';
            }   
        } 
...
<div class="container-fluid bg-light" style="padding: 50px;">
    <div class="container" style="width: 50%;">
            <div class="form-ordered">
                <h3 class="text-center"><strong>ORDERED NOW</strong></h3>
                ...
                <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                    ...
                    <fieldset class="form-group">
                        ...
                        <input type="text" class="form-control" id="address" placeholder="Địa chỉ" name="address" value="<?php echo $address ?>">
                    </fieldset>
                    ...
                    <fieldset class="form-group">
                        ...
                        <input type="text" class="form-control" id="phoneNumber" placeholder="Số điện thoại" name="tel" value="<?php echo $tel ?>">
                    </fieldset>     

                    <fieldset class="form-group">
                      <label>Loại vé:</label>
                      <span class="error ml-3">*<?php echo $typeErr;?></span>

                      <div class="d-flex justify-content-between">
                          <div class="form-check ml-5">
                            <input class="form-check-input" type="radio" name="type" id="type1" value="Thường"> 
                            <label class="form-check-label" for="type1">Vé thường</label>
                          </div>

                          <div class="form-check ml-5">
                            <input class="form-check-input" type="radio" name="type" id="type2" value="Cao cấp"> 
                            <label class="form-check-label" for="type2">Vé cao cấp</label>
                          </div>

                          <div class="form-check ml-5">
                            <input class="form-check-input" type="radio" name="type" id="type3" value="Đặc biệt"> 
                            <label class="form-check-label" for="type3">Vé đặc biệt</label>
                          </div>    
                      </div>                                           
                    ...
                    <fieldset class="form-group">
                        ...
                        <textarea class="form-control" id="note" rows="5" name="note" value="<?php echo $note ?>"></textarea>
                    </fieldset>
                    
                    <fieldset class="text-center">
                        <button type="submit" class="btn btn-outline-secondary">Đặt mua</button>
                    </fieldset>
